...
function ViTri() {
  ...
  const API_URL = variables.API_URL + "ViTri/";
  ...
  const loadData = async () => {
    try {
      ...
      const data = await res.json();
      setVitriList(data.data || []);
    } catch (err) {
      console.error("Lỗi load:", err);
      alert("⚠️ Không thể tải danh sách vị trí!");
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.vtName.trim()) {
      alert("⚠️ Tên vị trí không được để trống!");
      return;
    }
    ...
    try {
      const res = await fetch(url, {
        method,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ vtName: form.vtName }),
      });
      ...
      if (res.ok) {
        setForm({ vtName: "" });
        setEditId(null);
        await loadData(); // đảm bảo danh sách cập nhật trước khi alert
  ...
  const handleEdit = (item) => {
    setForm({ vtName: item.vtName });
    setEditId(item.vtid);
    alert("📝 Đang chỉnh sửa vị trí: " + item.vtName);
  };
    ...
    <div className="p-6">
      ...
      >
        <div className="flex-1">
          <label className="block text-gray-600 mb-1">Tên vị trí</label>
          <input
            type="text"
            name="vtName"
            value={form.vtName}
      ...
      <table className="min-w-full bg-white border rounded shadow">
        <thead className="bg-blue-100">
          <tr>
            ...
            <th className="border px-4 py-2 text-left">Tên vị trí</th>
        ...
        <tbody>
          {vitriList.length > 0 ? (
            vitriList.map((item) => (
              <tr key={item.vtid} className="hover:bg-gray-50">
                <td className="border px-4 py-2">{item.vtid}</td>
                <td className="border px-4 py-2">{item.vtName}</td>
                <td className="border px-4 py-2 text-center">
                  <button
                    onClick={() => handleEdit(item)}
